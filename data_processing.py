...
def train_and_generate_linear_regression_model(train_x, train_y, test_x, test_y):
    lr = LinearRegression()
    lr.fit(train_x, train_y['fare_amount'].to_list())
    joblib.dump(lr, 'fare_model_linear_regression.pkl')
    predicted_qualities = lr.predict(test_x)
    (rmse, mae, r2) = eval_metrics(test_y, predicted_qualities)
    return rmse, mae, r2


def train_and_generate_decision_tree_regressor_model(train_x, train_y, test_x, test_y):
    decision_tree_model = DecisionTreeRegressor()
    decision_tree_model.fit(train_x, train_y)
    joblib.dump(decision_tree_model, 'fare_model_decision_tree_regressor.pkl')
...
def process_data(data):
    ...
    train_x, train_y, test_x, test_y = split_data(selected_df)
